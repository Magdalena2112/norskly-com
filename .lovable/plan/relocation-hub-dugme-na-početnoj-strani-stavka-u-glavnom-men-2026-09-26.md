...
4. **Provera rada** — provera koda i izgradnje, pa test u pregledu na računaru (1280×1800) i telefonu (390×844): klik na novu stavku otvara stranicu, dugme „Nazad / Norskly” sa stranice vraća na početnu, ništa ne prelazi preko ivice ekrana.
...
- Nova stavka koristi `Link` (već je uvezen u toj datoteci) umesto običnog `href`, sa istom klasom koju koriste susedne stavke: `className="hover:text-primary transition-colors"`, redosled: Platforma → **Relocation Hub** → Za učenike → Za profesore → Kako učiš → FAQ.
