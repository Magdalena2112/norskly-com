...
const LessonBookedStudentEmail = ({ date, time, note }: Props) => (
  <Html lang="sr" dir="ltr">
    <Head />
    <Preview>Tvoj čas je zakazan — {date} u {time}</Preview>
    <Body style={main}>
      <Container style={container}>
        <Heading style={h1}>Čas je zakazan! 🎉</Heading>
        <Text style={text}>
          Tvoj čas norveskog jezika je uspešno zakazan.
        </Text>
        <Section style={detailsBox}>
          ...
          {note && (
            <>
              <Text style={detailLabel}>📝 Tvoja napomena</Text>
        ...
        <Hr style={hr} />
        <Text style={text}>
          Vidimo se uskoro! Ako treba da otkažeš, javi se na vreme.
        </Text>
        <Text style={footer}>— {SITE_NAME} tim</Text>
      </Container>
    </Body>
  </Html>
)
...
export const template = {
  component: LessonBookedStudentEmail,
  subject: (data: Record<string, any>) => `Čas zakazan: ${data.date || ''} u ${data.time || ''}`,
  displayName: 'Potvrda časa — učenik',
  previewData: { date: '15.01.2025', time: '14:00 – 15:30', note: 'Želim da vežbam razgovor' },
